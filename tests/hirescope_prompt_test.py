...
4. python hirescope_prompt_test.py
"""
...
1. 회사: 네이버클라우드 | 직무: 백엔드 개발자 | 직급: 시니어 | 기간: 2022.03 ~ 현재
   - 클라우드 인프라 API 서버 개발 및 운영
   - 모놀리식 아키텍처를 MSA로 전환하는 프로젝트 리드
   - 일일 API 호출량 500만 건 처리하는 시스템 안정화
   - 배포 파이프라인 개선으로 배포 시간 60% 단축

2. 회사: 카카오 | 직무: 서버 개발자 | 직급: 주니어 | 기간: 2020.01 ~ 2022.02
   - 카카오톡 선물하기 결제 시스템 백엔드 개발
   - Redis 캐싱 도입으로 API 응답 시간 40% 개선
   - 동시 접속자 10만명 대응 부하 테스트 및 최적화

3. 회사: 스타트업A | 직무: 웹 개발자 | 직급: 인턴 | 기간: 2019.06 ~ 2019.12
   - 사내 ERP 시스템 백엔드 개발 보조
   - REST API 설계 및 구현

[학력]
서울대학교 컴퓨터공학과 학사 졸업 (2016.03 ~ 2020.02)
...
[프로젝트]
1. MSA 전환 프로젝트 | 역할: 테크 리드 | 기간: 2023.01 ~ 2023.12
   - 사용 기술: Spring Cloud, Kubernetes, Kafka, gRPC
   - 성과: 서비스 장애 복구 시간 80% 단축, 배포 주기 주 1회에서 일 3회로 개선

2. 실시간 알림 시스템 | 역할: 메인 개발자 | 기간: 2022.06 ~ 2022.12
   - 사용 기술: Spring WebFlux, Redis Pub/Sub, WebSocket
   - 성과: 초당 5만건 메시지 처리, 메시지 지연시간 50ms 이하 달성

[자격증]
- AWS Solutions Architect Associate (2023.05)
- 정보처리기사 (2019.11)
"""
...
def call_llm(system_prompt: str, user_prompt: str, test_name: str):
    ...
    try:
        response = client.chat.completions.create(
            model="gemma-4-26b-a4b-it",  # LM Studio에 로드된 모델명 (다를 수 있음)
            messages=[
                {"role": "system", "content": system_prompt},
                {"role": "user", "content": user_prompt}
            ],
            temperature=0.3,  # 점수 일관성을 위해 낮게 설정
            max_tokens=2000,
        )
...
def main():
    ...
    if scoring_result:
        print(f"\n총점: {scoring_result.get('total_score', 'N/A')}")
    ...
    if summary_result:
        print(f"\n요약: {summary_result.get('summary', 'N/A')[:100]}...")
